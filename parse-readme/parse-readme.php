...
Class Automattic_Readme {
	...
	function parse_readme_contents( $file_contents ) {
		...
		// Last Updated
		if ( preg_match('|Last Updated:(.*)|i', $file_contents, $_last_updated) )
			$last_updated = $this->sanitize_text( $_last_updated[1] );
		else
			$last_updated = NULL;
		...
		foreach ( array('tags', 'contributors', 'requires_at_least', 'tested_up_to', 'stable_tag', 'donate_link', 'last_updated') as $chop ) {
			if ( $$chop ) {
				$_chop = '_' . $chop;
